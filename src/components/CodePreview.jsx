...
const Line = ({ children, indent = false }) => (
  <div className={`whitespace-pre-wrap leading-6 ${indent ? "ml-6" : ""}`}>{children}</div>
);
const SectionTitle = ({ children }) => (
  <div className="mt-4 mb-1 code-comment leading-6">{children}</div>
);
...
export default function CodePreview({ data, previewRef, onHeightChange, isLightMode }) {
  ...
  useEffect(() => {
    ...
    const compute = () => {
      // Contar elementos que tienen la clase leading-6 (nuestras líneas)
      const lineElements = el.querySelectorAll('.leading-6, .code-comment');
      ...
      <div className="flex">
        ...
        <div className="hidden md:block code-gutter gutter">
          {Array.from({ length: lineCount }).map((_, i) => (
            <div key={i} className="leading-6 text-right min-w-[2.5rem]">
              {i + 1}
            </div>
          ))}
        </div>
        ...
        <div 
          ref={contentRef}
          className="print-scale code-content w-full flex flex-col items-center"
        >
          ...
          <div className="w-full max-w-5xl mx-auto">
            ...
            {/* Dos columnas centradas con justificación */}
            <div className="two-col grid grid-cols-2 gap-8 justify-center text-justify">
              {/* Columna izquierda */}
